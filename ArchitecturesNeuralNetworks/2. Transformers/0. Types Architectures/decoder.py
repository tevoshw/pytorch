"""
Decoder ALGORITHM BY SCRATCH IN PYTORCH, the main ideia and structure:

1. Embedding (embdding + pos embedding)
2. TransformerBlock(
    - Attention (Self attetion +  MASK IN THE SCORES) + residual connection and normalization
    - FeedForwardNetwork(Normal dense network, with non-linearity) + residual connection and normalization
)
3. Final Dense (The final layer to predict the new token, NUM_TOKENS)

So basically different from ENCODER, an DECODER can predict tokens and not classes
...
class MultiHeadAttention(nn.Module):
    ...
    """
        Class MultiHeadAttention:
            Self multi head attetion algorithm from scratch with masked (DECODER)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        scores = (Q @ K.tranpose(-1, -2)) / math.sqrt(d_k) # Here calculate sthe scores and normalize them
        mask = torch.tril(torch.ones(t, t, dtype = bool)) # Create the Mask 
        scores = scores.masked_fill(mask == False, '-inf') # Applie the mask
...
class BERT(nn.Module):
    """
        ...
        Output:
            The output gonna be an (BATCH, VOCAB_SIZE)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
            ...
            logits = self.logits(x) # (B, VOCAB_SIZE)
